...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, n_heads, dropout=0.1):
        ...
        assert d_model % n_heads == 0, "d_model % n_heads != 0, init failed"
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, inputs, attn_mask):
        ...
        residual = inputs
        # Layer Normalization
        lm = self.layer_norm1(inputs)

        # multi-head attention
        # attn: [batch_size, len_q, d_model]
        attn, attn_weights = self.multi_attention(lm, lm, lm, attn_mask)

        # Residual Dropout
        attn = self.dropout(attn)

        # Layer Normalization
        ffn_outputs = self.layer_norm2(residual + attn)

        # Feed Forward Network
        # ffn_outputs : [batch_size, len_q, d_model]
        ffn_outputs = self.ffn(ffn_outputs)

        ffn_outputs = residual + attn + ffn_outputs
...
class GPT2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # outputs: [batch_size, len_q, d_model]
        # attention_weights: [batch_size, n_heads, len_q, len_k]
        return outputs, attention_weights
    ...
    def generate(self, inputs, max_len=20):
        ...
        for _ in range(max_len):
            # outputs: [batch_size, len_q, d_model]
            outputs, attention_weights = self.decoder(outputs)
            # outputs: [batch_size, len_q, vocab_size]
            outputs = torch.softmax(outputs, dim=-1)
            # outputs: [batch_size, len_q]
            outputs = torch.multinomial(outputs[:, -1], 1)
            # inputs: [batch_size, len_q + 1]
...
class GPT2LMHead(nn.Module):
    def __init__(self, gpt2) -> None:
        ...
        vocab_size, d_model = gpt2.decoder.embedding.weight.size()
        self.gpt2 = gpt2
        self.linear = nn.Linear(d_model, vocab_size, bias=False)

        self.linear.weight = self.gpt2.decoder.embedding.weight

    def forward(self, inputs):
        # inputs: [batch_size, len_q]
        outputs, attention_weights = self.gpt2(inputs)
        # outputs: [batch_size, len_q, vocab_size]
