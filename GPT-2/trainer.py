...
class Trainer:
    ...
    def train(self, epoch):
        try:
            if self.do_pretrain:
                if self.train_log.enable:
                    ...
                    if epoch == 1:
                        ...
                        self.train_log.logger.info(f"model parameters: {total_num}")
                self.pretrain(epoch)
            elif self.do_eval:
                self.evaluate(epoch)
            ...
            if self.train_log.enable:
                self.train_log.logger.info("KeyboardInterrupt")
        except Exception as e:
            if self.train_log.enable:
                self.train_log.logger.exception(e)
            raise e
    ...
    def pretrain(self, epoch):
        ...
        for step, batch in enumerate(tqdm(self.train_loader, desc="Iteration")):
            ...
            if epoch == 1 and self.training_args.local_rank == 1:
                # I want to see the input and target of the first batch
                if self.train_log.enable:
